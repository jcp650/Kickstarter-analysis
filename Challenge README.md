...
One limitation that I encountered was a lack of information on the client's strategy for the *Fever* Kickstarter campaign. Because there  was no campaign length provided, I could not effectively analyze how the length of the campaign would effect the potential outcome. I could have subtracted the deadline from the launch date and created a table similar to figure 2 for the success rate based on campaign length. Calculating the success rate based on campaign length would be a highly valuable metric to accompany the success rate based on launch date. Further, there was no data provided by the client regarding the campaign progress and the amount of money raised. To truly compare the performance of the *Fever* campaign with similar campaigns, there would need to be more complete data including campaign duration and exact amount pledged thus far. Finally, there was no data dictionary provided with the Kickstarter data, which would have been helpful in accurate analysis. Referencing a data dictionary would provide clear definitions and methods used for gathering the variables, which would produce more cohesive and precise analysis.
